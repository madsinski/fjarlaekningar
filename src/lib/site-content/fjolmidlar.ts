...
export const FJOLMIDLAR_FIELDS: SiteField[] = [
  ...
  { key: "hero_body", label: "Undirtexti", group: "Hetjusvæði", type: "textarea" },
  {
    ...
    type: "textarea",
    help:
      "Snið: Fyrirsögn | Miðill | Dagsetning | Slóð | Tegund | Útdráttur | Mynd. " +
      "Tegund er frett, vidtal eða skodun. Útdrátturinn er okkar eigin lýsing, ekki texti úr greininni. " +
      "Mynd er slóð á mynd í /press/ og birtist aðeins á efstu færslunni. Allt frá Tegund og aftur úr má sleppa. " +
...
const ITEMS_EN = [
  "HSU og Fjarlækningar hefja samstarf um nýja stafræna læknisþjónustu | Heilbrigðisstofnun Suðurlands | 2026-08-13 | https://island.is/s/hsu/frett/hsu-og-fjarlaekningar-hefja-samstarf-um-nyja-stafraena-laeknisthjonustu | frett | A pilot project between HSU and Fjarlækningar in Vestmannaeyjar. HSU's announcement states that patients aged 18 and over can submit a case through the portal and hear back from a doctor within two hours during opening hours, 10:00–22:00. | /press/hsu-fjarlaekningar.webp",
...
export type PressItem = {
  ...
  summary: string;
  /** Path to a local image, shown on the lead card only. May be empty. */
  image: string;
};

/** Credit line under a press photo — the outlet the picture came from. */
export const PHOTO_CREDIT = { is: "Mynd", en: "Photo" } as const;

/**
 * Parse "Fyrirsögn | Miðill | Dagsetning | Slóð | Tegund | Útdráttur | Mynd"
 * rows. Everything after the URL is optional, so lists written before those
 * columns existed keep parsing unchanged. Rows without a title or a valid URL
 * are dropped.
 */
export function pressItems(c: LocaleContent): PressItem[] {
  return (c.items ?? "")
    ...
    .map(([title, outlet, date, url, kind, summary, image]) => ({
      title,
      outlet,
      date,
      url,
