...
import { DEFAULT_LANG, translator, type Lang } from "./i18n/core";
...
export function cleanShiftType(body: Record<string, unknown>, partial: boolean, lang: Lang = DEFAULT_LANG): Record<string, unknown> | string {
  const t = translator(apiAdmin, lang);
  const p: Record<string, unknown> = {};
  if (typeof body.name === "string" && body.name.trim()) p.name = body.name.trim().slice(0, 60);
  else if (!partial) return t("type.nameMissing");
  if (typeof body.short === "string") p.short = body.short.trim().slice(0, 8);
  if (typeof body.starts === "string") { if (!TIME_RE.test(body.starts)) return t("type.badStart"); p.starts = asHm(body.starts); }
  if (typeof body.ends === "string") { if (!TIME_RE.test(body.ends)) return t("type.badEnd"); p.ends = asHm(body.ends); }
  ...
  if (p.on_holidays && p.skip_holidays) return t("type.holidayConflict");
