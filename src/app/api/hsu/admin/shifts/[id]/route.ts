...
import { applyShiftChanges } from "@/lib/hsu/shift-edit";
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return fail("Ógild beiðni");
  const { data: shift } = await supabaseAdmin.from("hsu_shifts").select("doctor_id, shift_date, published").eq("id", id).maybeSingle();
  if (!shift) return fail("Vaktin fannst ekki", 404);
