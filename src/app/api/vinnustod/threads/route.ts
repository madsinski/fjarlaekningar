// Spurningar til stjórnanda Fjarlækninga: listi og ný spurning. Allir sem
// komast inn í vinnustöðina spyrja — nema stjórnandinn sjálfur, sem svarar.
...
export async function POST(req: Request) {
  ...
  const { data: thread, error } = await supabaseAdmin.from("gatt_threads")
    .insert({ ...ownerFields(actor), subject }).select("id").single();
