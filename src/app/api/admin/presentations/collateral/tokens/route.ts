...
// collateral. Backed by supabase/presentations-studio-schema.sql.
...
// POST — mint a new token (admin). Returns the plaintext token once.
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabaseAdmin
    ...
    .insert({ token_hash: hash(token), label: body.label ?? null, created_by: caller!.id });
