...
import { Button, Field, Notice, hsuApi, inputCls } from "@/app/hsu/_components/ui";
import { FjLogo } from "../../_components/shared";
...
export default function ActivateClient({ token }: { token: string }) {
  ...
  useEffect(() => {
    (async () => {
      const r = await hsuApi<{ name: string; email: string; reset: boolean }>(`/api/vinnustod/auth/invite?token=${encodeURIComponent(token)}`);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const r = await hsuApi("/api/vinnustod/auth/invite", { body: { token, password: pw, pin: usePin ? pin : "" } });
    setBusy(false);
    if (!r.ok) { setErr(r.error ?? "Ekki tókst að virkja aðganginn"); return; }
    window.location.href = "/vinnustod";
  };
    ...
    <main className="flex min-h-screen flex-col items-center justify-center px-4 py-10">
      <div className="w-full max-w-md">
        <div className="mb-6 flex items-center justify-center gap-3">
          <FjLogo size={40} />
          <div>
            <div className="text-sm font-bold text-slate-900">Vinnustöð Fjarlækninga</div>
            <div className="text-xs text-slate-500">Fyrir starfsfólk heilsugæslunnar</div>
          </div>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {invalid && (
            <div className="space-y-4 text-center">
              ...
              <p className="text-sm text-slate-600">{invalid} Skráðu þig aftur eða notaðu „Gleymt lykilorð“ á innskráningarsíðunni.</p>
              <a href="/vinnustod" className="inline-block text-sm font-semibold text-[var(--hsu)] hover:underline">Fara á innskráningu</a>
            </div>
          )}
          {info && (
            <form onSubmit={submit} className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Þú skráir þig inn með <span className="font-semibold text-slate-900">{info.email}</span>.
                </p>
              </div>
